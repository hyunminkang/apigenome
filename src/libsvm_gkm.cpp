...
static void kmertree_dfs_pthread_process(kmertree_dfs_pthread_t *td, const int nthreads, const int start, const int end, double *res)
{
    ...
    for (i=1; i<nthreads; i++) {
        ...
        if (rc[i]) {
	  error("failed to create thread. pthread_create() returned %d", rc[i]);
        } else {
	  notice("thread %d was created.", i);
        }
    }
...
static double kmertreecoef_dfs_pthread_process(kmertreecoef_dfs_pthread_t *td, const int nthreads)
{
    ...
    for (i=1; i<nthreads; i++) {
        ...
        if (rc[i]) {
	  error("failed to create thread %d. pthread_create() returned %d", i, rc[i]);
        } else {
          notice("thread %d was created", i);
        }
    }
...
static void gkmkernel_build_mmcnt_lookuptable()
{
    ...
    if (g_param->L <= MMCNT_LOOKUPTAB_WIDTH) {
      ...
      error("L(%d) cannot be greater than MMCNT_LOOKUPTAB_WIDTH*2 (%d).", g_param->L, MMCNT_LOOKUPTAB_WIDTH*2);
    }

    notice("g_mmcnt_lookuptab_mask: %x", g_mmcnt_lookuptab_mask);
    notice("g_mmcnt_nlookups: %d", g_mmcnt_nlookups);
}
...
gkm_data* gkmkernel_new_object(const char *seq, const char *sid, int seqid)
{
    gkm_data *d;
    int i, j, k;
    ...
    for (i=0; i<d->seqlen; i++) {
        switch (toupper(seq[i])) {
            ...
            default: 
                d->seq[i] = 1; 
                //warning("'%c' at %s(%d) is not a valid nucleotide. Only ACGT are allowed", seq[i], sid, i);
                break;
        }
    }
    ...
    */
    ...
    //notice("%d's sqnorm is %f", seqid, d->sqnorm);
...
void gkmkernel_init(struct svm_parameter *param)
{
    ...
    notice("gkm-kernel weights:");

    for (i=0; i<=g_param->d; i++) {
      notice("  c[%d] = %.6f", i, g_weights[i]);     
    }
...
double* gkmkernel_kernelfunc_batch(const gkm_data *da, const union svm_data *db_array, const int n, double *res) 
{
    ...
    notice("add sequences to kmertree (%ld ms)", diff_ms(time_end, time_start));
    ...
    gettimeofday(&time_end, NULL);
    notice("DFS n=%d (%ld ms)", n, diff_ms(time_end, time_start));
...
double* gkmkernel_kernelfunc_batch_all(const int a, const int start, const int end, double *res) 
{
    ...
    gettimeofday(&time_end, NULL);
    notice("DFS i=%d, start=%d, end=%d (%ld ms)", a, start, end, diff_ms(time_end, time_start));
...
double* gkmkernel_kernelfunc_batch_sv(const gkm_data *da, double *res) 
{
    if (g_sv_kmertree == NULL) {
      error("kmertree for SVs has not been initialized. call gkmkernel_init_sv() first.");
      return NULL;
    }
    ...
    gettimeofday(&time_end, NULL);
    notice("DFS nSVs=%d (%ld ms)", g_sv_num, diff_ms(time_end, time_start));
...
double gkmkernel_predict(const gkm_data *d)
{
    ...
    if (g_sv_kmertreecoef == NULL) {
      error("kmertreecoef has not been initialized. call gkmkernel_init_sv() first.\n");
      return 0;
    }
    ...
    if (g_param_nthreads == 1) {
      ...
      warning("Supported number of threads are 1, 4 and 16. nthread is set to 1");
      g_param_nthreads = 1;
      result = kmertreecoef_dfs_single(d);
    }
...
void gkmkernel_set_num_threads(int n)
{
    ...
    notice("Number of threads is set to %d", n);
...
static bool read_model_header(FILE *fp, svm_model* model)
{
    ...
    while(1)
    {
        ...
        else if(strcmp(cmd,"kernel_type")==0)
        {
            ...
            {
	      error("unknown kernel function (%s).", cmd);
                return false;
            }
        }
        ...
        {
	  error("unknown text in model file: [%s]",cmd);
            return false;
        }
    }
