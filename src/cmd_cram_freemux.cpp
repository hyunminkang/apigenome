...
int32_t cmdCramFreemux(int32_t argc, char** argv) {
  ...
  htsFile* wGLs = hts_open((outPrefix+".gls").c_str(),"w");
  
  if ( wGLs == NULL )
    error("[E:%s:%d %s] Cannot create %s.gls file",__FILE__,__LINE__,__FUNCTION__,outPrefix.c_str());    
  ...
  for(i=0, k=0; i < scl.nsnps; ++i) {
	...
	GLs[2] /= tmp;
      ...
      hprintf(wGLs,"%d\t%d\t%.3lg\t%.3lg\t%.3lg\n", it->first, i, GLs[0]/tmp, GLs[1]/tmp, GLs[2]/tmp);
    }
  }
