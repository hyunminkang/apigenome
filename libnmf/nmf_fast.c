...
double nmf_fast(double ** a, double ** w0, double ** h0, int m, int n, \
		      int k, int * maxiter, const double TolX, const double TolFun) 
{
  ...
  double * hht = (double*) malloc(sizeof(double) * k*k);
  double * aht = (double*) malloc(sizeof(double) * m*k);
  double * gw  = (double*) malloc(sizeof(double) * m*k);   //gradient of w
  ...
  double * wtw = (double*) malloc(sizeof(double) * k*k);
  double * wta = (double*) malloc(sizeof(double) * k*n);
  double * gh  = (double*) malloc(sizeof(double) * k*n);   //gradient of h
  ...
  for (iter = 1; iter <= *maxiter; ++iter) {
	    ...
	    //----------------------------
	    //hht = h*h'
	    dgemm('N', 'T', k, k, n, 1.0, *h0, k, *h0, k, 0., hht, k);
	    //aht= a*h'
	    dgemm('N', 'T', m, k, n, 1.0, *a, m, *h0, k, 0., aht, m);
	    ...
	    for (innerIter = 1; innerIter <= maxInnerIter; ++innerIter) {
	      ...
	      dlacpy('A', m, k, aht, m, gw, m);
	      dgemm('N', 'N', m, k, k, 1.0, *w0, m, hht, k, -1.0, gw, m);
	      

	      

	      double minwgw; // to store minimum of w and gw
		...
		//workspace query
		double work_wupd_query = 0.;
		int lwork_wupd = 0;
		double * work_wupd;
		dsytrf('U', rowsHelp3, help1, rowsHelp3, jpvt_h, &work_wupd_query, -1, &info);
		lwork_wupd = (int) work_wupd_query;
		work_wupd = (double*) malloc(sizeof(double) * lwork_wupd);
		dsytrf('U', rowsHelp3, help1, rowsHelp3, jpvt_h, work_wupd, lwork_wupd, &info);
		dsytrs('U', rowsHelp3, colsHelp3, help1, rowsHelp3, jpvt_h, help3, rowsHelp3, &info);
	    ...
	    //----------------------------
	    //wtw = w' * w
	    dgemm('T', 'N', k, k, m, 1.0, *w0, m, *w0, m, 0., wtw, k);
	    //wta = w' * a
	    dgemm('T', 'N', k, n, m, 1.0, *w0, m, *a, m, 0., wta, k);
	    ...
	    for (innerIter = 1; innerIter <= maxInnerIter; ++innerIter) {
	      ...
	      dlacpy('A', k, n, wta, k, gh, k);
	      dgemm('N', 'N', k, n, k, 1.0, wtw, k, *h0, k, -1.0, gh, k);
	      ...
	      double minhgh; // to store minimum of h and gh
		...
		double work_hupd_query = 0.;
		int lwork_hupd = 0;
		double *work_hupd;
		dsytrf('U', rowsHelp2, help1, rowsHelp2, jpvt_h, &work_hupd_query, -1, &info);
		lwork_hupd = (int) work_hupd_query;
		work_hupd = (double*) malloc(sizeof(double) * lwork_hupd);
		dsytrf('U', rowsHelp2, help1, rowsHelp2, jpvt_h, work_hupd, lwork_hupd, &info);
		dsytrs('U', rowsHelp2, colsHelp2, help1, rowsHelp2, jpvt_h, help2, rowsHelp2, &info);
