...
int pg_subprob_w_singleprec(float *a, float *w0, float *h, float *grad, float *wn, float *aht, float *hht, float * d, float * tempw, int m, int n, int k, float tol, int maxiter) 
{
  

  float * wp = (float*) malloc(sizeof(float)*m*k);
  int decr_factor_a;

  //aht = A * H'
  sgemm('N', 'T', m, k, n, 1.0, a, m, h, k, 0., aht, m);

  //hht = H * H'
  sgemm('N', 'T', k, k, n, 1.0, h, k, h, k, 0., hht, k);
  ...
  for (iter=1; iter<= maxiter; ++iter) {
    //grad = W * HHt - AHt
    slacpy('A', m, k, aht, m, grad, m);
    sgemm('N', 'N', m, k, k, 1.0, w0, m, hht, k, -1.0, grad, m);
    ...
    for (inner_iter = 1; inner_iter <= 20; ++inner_iter) {
      ...
      // copy w0 to wn
      slacpy('A', m, k, w0, m, wn, m);
      // compute wn = - factor_a * grad + wn
      saxpy(m*k, - factor_a, grad, 1, wn, 1);
	...
	if (wn[i] < ZERO_THRESHOLD)
      ...
      //copy wn to d
      slacpy('A', m, k, wn, m, d, m);



      // d = wn - w0
      saxpy(m*k, - 1.0, w0, 1, d, 1);
	...
	gradd += grad[i] * d[i];
      ...
      //dqd = sum(sum((hht*d).*d))
      //tempw = d * hht = m x k
      sgemm('N', 'N', m, k, k, 1.0, d, m, hht, k, 0., tempw, m);
	...
	if (!suff_decr || wpeqwn) {
	  //w = wp;
	  slacpy('A', m, k, wp, m, w0, m);
	  break;
	}
	else {
	  factor_a = factor_a/factor_b;
	  //wp = wn;
	  slacpy('A', m, k, wn, m, wp, m);
	}
      }
    }
  }
