...
int pg_subprob_h_singleprec(float *a, float *w, float *h0, float *grad, float *hn, float *wta, float *wtw, float * d, float * temph, int m, int n, int k, float tol, int maxiter) 
{
  ...
  //wta = W' * A;
  sgemm('T', 'N', k, n, m,  1.0, w, m, a, m, 0., wta, k);

  //wtw = w'*w;
  sgemm('T', 'N', k, k, m, 1.0, w, m, w, m, 0., wtw, k);
  ...
  for (iter=1; iter<= maxiter; ++iter) {
    //grad = wtw*h-wta;
    slacpy('A', k, n, wta, k, grad, k);
    
    sgemm('N', 'N', k, n, k, 1.0, wtw, k, h0, k, - 1.0, grad, k);
    ...
    for (inner_iter = 1; inner_iter <= 20; ++inner_iter) {
      ...
      // copy h0 to hn
      slacpy('A', k, n, h0, k, hn, k);
      // computy hn = - factor_a * grad + hn
      saxpy(k*n, - factor_a, grad, 1, hn, 1);
	...
	if (hn[i] < ZERO_THRESHOLD)
      ...
      //copy hn to d
      slacpy('A', k, n, hn, k, d, k);

      // d = hn - h0
      saxpy(k*n, - 1.0, h0, 1, d, 1);
	...
	gradd += grad[i] * d[i];
      ...
      //dqd = sum(sum((wtw*d).*d))
      sgemm('N', 'N', k, n, k, 1.0, wtw, k, d, k, 0., temph, k);
      float dqd = 0;
      for (i=0; i< k*n; ++i) {
	dqd += temph[i]*d[i];
      }
	...
	if (!suff_decr || hpeqhn) {
	  //h = hp;
	  slacpy('A', k, n, hp, k, h0, k);
	  break;
	}
	else {
	  factor_a = factor_a/factor_b;
	  //hp = hn;
	  slacpy('A', k, n, hn, k, hp, k);
	}
      }
    }
  }
