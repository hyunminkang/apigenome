...
void generateMatrix(const int m, const int n, const int k, init_t init, const int min, const int max, double **matrixW, double **matrixH, double * matrixA, options_t * opts)
{
  ...
  if (init == init_infogain || init ==  init_gainratio) {
	...
	init = init_rand;
      ...
      idx_double * indexedInfoGainSorted = (idx_double*) malloc(sizeof(idx_double) * n);
      idx_double * indexedGainRatioSorted = (idx_double*) malloc(sizeof(idx_double) * n);
    
      int error = 0;
      if (opts->parallelization == par_openmp) {
	error = infoGain_openmp(matrixA, m, n, opts->classColumn, opts->num_dist_classes, indexedInfoGainSorted, indexedGainRatioSorted);
      }
      else {
	error = infoGain(matrixA, m, n, opts->classColumn, opts->num_dist_classes, indexedInfoGainSorted, indexedGainRatioSorted);
      }      
	...
	for (i = 0; i < k; ++i) {
	  ...
	  dcopy(m, matrixA + srcRow * m, 1, (*matrixW) + i * m, 1);
	}
      }
      else {
	for (i = 0; i < k; ++i) {
