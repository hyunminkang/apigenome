...
int calculateSVD_singleprec(float * a, float * u, float * s, float * v, int m, int n, int k, int maxiter, float tol, int ncv, int order)
{
  ...
  workl = (float*) malloc(sizeof(float) * ncv * (ncv + 8));	//working space matrix
  workd = (float*) malloc(sizeof(float)*3*(mindim));			//working space matrix
  resid = (float*) malloc(sizeof(float)*maxdim);			//residual vector
  ax = (float*) malloc(sizeof(float)*maxdim);				// used for computing matrix vector products
  ...
  while(1){
    ...
    //call dsaupd
    if (m >= n) {
      ssaupd(&ido, bmat, mindim, which, nev, tol, resid, ncv, v, ldv, iparam, ipntr, workd, workl,lworkl, &info );
    }
    else {
      ssaupd(&ido, bmat, mindim, which, nev, tol, resid, ncv, u, m, iparam, ipntr, workd, workl,lworkl, &info );
    }
    ...
    if (m >= n) {
      sgemv('N', m, n, 1.0, a, m, workd + ipntr[0] - 1, 1, 0.0, ax, 1); 
      sgemv('T', m, n, 1.0, a, m, ax, 1, 0.0, workd + ipntr[1] - 1, 1);
    }
    else {
      sgemv('T', m, n, 1.0, a, m, workd + ipntr[0] - 1, 1, 0.0, ax, 1); 
      sgemv('N', m, n, 1.0, a, m, ax, 1, 0.0, workd + ipntr[1] - 1, 1);
    }
  }
  ...
  if (info < 0) {
    ...
    rvec = 1;
    if (m >= n) {
      sseupd( rvec, 'A', select, s, v, ldv, sigma, bmat, mindim, which, nev, tol, resid, ncv, v, ldv, iparam, ipntr, workd, workl, lworkl, &ierr);
    }
    else {
      sseupd( rvec, 'A', select, s, u, m, sigma, bmat, mindim, which, nev, tol, resid, ncv, u, m, iparam, ipntr, workd, workl, lworkl, &ierr);
    }
  }
...
//               %-----------------------------%
      ...
      if (m >= n) {
        sgemv('N', m, n, 1.0, a, m, v + n*j, 1, 0.0, ax, 1);	//ax = A*v
        scopy(maxdim, ax, 1, u + j*m, 1);
        temp = 1.0/snrm2(maxdim, u + j*m, 1);
        sscal(maxdim, temp, u + j*m, 1);
      }
      else {
	sgemv('T', m, n, 1.0, a, m, u + m*j, 1, 0.0, ax, 1);	//ax = A'*u
        scopy(maxdim, ax, 1, v + j*n, 1);
        temp = 1.0/snrm2(maxdim, v + j*n, 1);
        sscal(maxdim, temp, v + j*n, 1);
      }
  ...
  if (order && (k > 1) ) {
    ...
    for (index = 0; index < (k / 2); ++ index)
      sswap(m, u + index * m, 1, u + (k - index - 1) * m, 1);
    ...
    for (index = 0; index < (nev / 2); ++ index)
      sswap(n, v + index * n, 1, v + (k - index - 1) * n, 1);    
  }
