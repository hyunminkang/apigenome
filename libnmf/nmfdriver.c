  ...
  for (repetitions = 1; repetitions <= opts->rep; ++repetitions) {
    switch (alg) {
      ...
      case pg:	norm = nmf_pg(&A, &W0, &H0, m, n, k, &iter, opts->TolX);
		break;
      case bayes: norm = nmf_bayes(&A, &W0, &H0, m, n, k, &iter, opts->TolX, opts->TolFun);
		break;
      case fast: norm = nmf_fast(&A, &W0, &H0, m, n, k, &iter, opts->TolX, opts->TolFun);
		break;		
    }
    ...
    if (repetitions < opts->rep) {
      generateMatrix(m, n, k, init_rand, opts->min_init, opts->max_init, &W0, &H0, A, opts);
    }
